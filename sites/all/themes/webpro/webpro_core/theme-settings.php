 ...
 */
function webpro_core_form_system_theme_settings_alter(&$form, $form_state) {
  global $base_url;
  ...
  // Get defaults from webpro core if not defined in .info.
  if($theme_name != 'webpro_core') {
  $webpro_defaults = $theme_data['webpro_core']->info['settings'];
    foreach($webpro_defaults as $key=>$setting) {
  ...
  $width_options = webpro_core_get_width_options($theme_name);

  // webpro theme settings container
  $form['tnt_container'] = array(
    '#type' => 'fieldset',
    '#title' => t('webpro theme settings'),
    '#description' => t('Use these settings to enhance the appearance and functionality of your webpro theme.'),
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
    '#weight' => -1,
    '#attached' => array(
      'css' => array(drupal_get_path('theme', 'webpro_core') . '/css/settings-form.webpro-core.css'),
    ),
  );
  ...
  $form['tnt_container']['general_settings']['theme_responsive_desktop'] = array(
    ...
    '#description' => t('Only this layout will be used.  For tablet and smartphone layouts, enable the <a href="@accelerator">webpro Accelerator module</a>.',
      array('@accelerator' =>
        url('http://drupal.org/project/webpro_accelerator',
        array('external' => TRUE))
      )
    ),
    '#weight' => -6,
  ...
  /**
   * If webpro Accelerator is unavailable, provide webpro 1.x style grid options.
   * Toggles are based of FAPI #states, but the description is slightly modified
   * based on module availability.
   */
  if (!module_exists('webpro_accelerator')) {
    $form['tnt_container']['general_settings']['theme_grid_config']['#description'] .= ' ' .
      ...
      the <a href="@accelerator">webpro Accelerator module</a> to support responsive layouts.</p>',
      array('@accelerator' =>
        url('http://drupal.org/project/webpro_accelerator',
        array('external' => TRUE))
        )
...
/**
 * @param (string) $theme
 *    Machine name of webpro theme.
 * @return
 *    Array of sidebar widths options.
 */
function webpro_core_get_width_options($theme) {
